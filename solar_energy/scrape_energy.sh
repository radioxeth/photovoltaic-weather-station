...
while [[ "$current_date" < "$end_date" ]]; do
    ...
    # Formatted filename
    filename="data/energy-${year_month}.csv"

    # Construct the URL
    url="https://monitoringapi.solaredge.com/site/$SOLAR_SITE_ID/energy?timeUnit=QUARTER_OF_AN_HOUR&startDate=${current_date}&endDate=${last_day}&api_key=$SOLAR_API_KEY&format=csv"
...
# combine all the csv files into one named solar_energy_data.csv and remove the header row
echo date,value > data/solar_energy_data.csv
for file in data/energy-*.csv; do
    tail -n +2 $file >> data/solar_energy_data.csv
done
